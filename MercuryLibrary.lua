...
function Library:toggle(options)
	...
	function Ttable:Toggle()
		toggle()
	end

	function Ttable:SetState(state)
		toggled = state
		if toggled then
			offIcon:crossfade(onIcon, 0.1)
		else
	...
	if options.StartingState then Ttable:SetState(true) end
...
function Library:labelwithdesc(options)
